...
% library. This code repo is typically located at: (not right now)
%
%   https://github.com/ivsg-psu/FieldDataCollection_VisualizingFieldData_PlotTestTrack
%
...
%% fcn_INTERNAL_printMatrixWithSpecificNumberOfDecimalPoints

function fcn_INTERNAL_printMatrixWithSpecificNumberOfDecimalPoints(matrix_to_print,num_decimal_places_each_column)

if length(num_decimal_places_each_column)~=length(matrix_to_print(1,:))
    error('The number of columns in the matrix does not match the input specifying the number of decimal places in each column.')
end

N_rows = length(matrix_to_print(:,1));
N_cols = length(matrix_to_print(1,:));

fprintf(1,'\n\n');
for ith_row = 1:N_rows
    for jth_col = 1:N_cols
        format_string = sprintf('%%.%.0df  ',num_decimal_places_each_column(jth_col));
        fprintf(1,format_string, matrix_to_print(ith_row,jth_col));
    end
    fprintf(1,'\n');
end
